...
- **Autentikasi SSH pakai Key + Generate Key (Fase 26)** — 🔶 **IMPLEMENTASI berjalan** (§26, uji native pending 2026-08-26): **26A ✅** pakai private key existing (Browse + passphrase keychain + key tak ikut sync §26.10) · **26B ✅** generate keypair Ed25519/RSA in-app (`keygen.rs` + `GenerateKeyModal`) · **26D-1 ✅** multi-method Auto (publickey→password satu koneksi). Sisa: **26D-2** keyboard-interactive/2FA · **26D-3** agent (ssh-agent/Pageant) · 26C install-ke-server (prioritas rendah). Branch kerja `koneksiSSH-withSSHKey`.
...
|---|---|---|
| **26A ✅ — Pakai key existing + strip sync** | Browse file key · checkbox+input passphrase (simpan keychain) · kirim passphrase ke backend (utama + jump host) · **strip `keyPath` dari sync + preserve keyPath lokal saat pull (§26.10)**. **Uji: connect pakai key ber-passphrase · push→pull di device lain key kosong** | `ProfileEditor.tsx` · `profiles.ts` · `App.tsx` · `sync.rs` |
| **26B ✅ — Generate keypair** | Command `ssh_generate_keypair` (Ed25519/RSA, passphrase, chmod 600) · `GenerateKeyModal` · tulis `id_*`+`.pub` · set profil · tampil+Copy public key. **Uji: generate → file muncul → public key valid** | **baru** `keygen.rs` · **baru** `GenerateKeyModal.tsx` · `ProfileEditor.tsx` · `lib.rs` · `Cargo.toml` |
...
| **26D-1 ✅ — Multi-method Auto** | Backend coba `publickey` (bila key ada) → `password` dalam satu koneksi; varian `SshAuth::Auto{key?,password?}`; "Auto" di UI kirim key+password | `ssh.rs` · `profiles.ts` |
| **26D-2 — Keyboard-interactive (2FA)** | Varian `SshAuth::Interactive`; relay prompt server ↔ UI (modal prompt dinamis via event); masuk ke rantai Auto sebagai fallback terakhir | `ssh.rs` · UI prompt baru |
| **26D-3 — Agent (ssh-agent/Pageant)** | Varian `SshAuth::Agent`; konek agent (named pipe Windows / socket Unix via `russh::keys::agent`), auth pakai identity agent; masuk rantai Auto | `ssh.rs` · `ProfileEditor.tsx` |
...
**Status:** rancangan (diskusi user 2026-08-25, penyimpanan/sync dikunci 2026-08-26). **26A ✅ + 26B ✅ + 26D-1 ✅ implementasi (uji native pending, 2026-08-26)**. Sisa (keputusan user 2026-08-26 — relevan krn punya server 2FA + pakai agent): **26D-2** keyboard-interactive (2FA), **26D-3** agent (ssh-agent/Pageant); **26C** install-ke-server prioritas rendah.

**Progress:**
- **26D-1 ✅ (implementasi; uji native pending, 2026-08-26)** — Multi-method "Auto". **Backend `ssh.rs`**: varian baru `SshAuth::Auto { key: Option<AutoKey>, password: Option<String> }` (+struct `AutoKey{path,passphrase?}`). Arm auth-nya coba **publickey dulu** (bila key ada & bisa di-load — gagal-load di-skip, bukan fatal) lalu **fallback password**, keduanya di **koneksi yang sama** (russh membolehkan beberapa percobaan auth berturut). **Frontend `profiles.ts`**: `sshOpenFromProfile` untuk `authMethod==="auto"` kini kirim `{type:"auto", key:{path,passphrase}|null, password|null}` (password dari keychain/`s.password`, passphrase key dari keychain 26A) — bukan lagi cuma memilih salah satu. `authMethod` `password`/`key` tak berubah; `agent`/`interactive` masih jatuh ke password (menyusul 26D-3/26D-2). **Verifikasi:** `tsc` lolos; `cargo check` **dan** `--features msstore` lolos. Sisa: uji native — profil "Auto" dgn key valid → login via key; key salah/absen → jatuh ke password.
- **26B ✅ (implementasi; uji native pending, 2026-08-26)** — Generate keypair in-app. **Backend `src-tauri/src/keygen.rs`** (baru): command `ssh_generate_keypair(algo, bits?, passphrase?, comment?, out_path, overwrite?)` pakai `russh::keys::ssh_key` (crate `ssh-key` 0.7.0-rc.11 yang sudah ditarik russh — **tanpa shell-out `ssh-keygen`**). Ed25519 via `PrivateKey::random(rng, Algorithm::Ed25519)`; RSA via `RsaKeypair::random(rng, bits)`→`PrivateKey::from` (default 3072, batas 2048–8192). Passphrase → `key.encrypt(rng, pw)` (AES256; hanya bila diisi). Tulis private (`to_openssh(LineEnding::LF)`) + `<path>.pub` (`public_key().to_openssh()`), **chmod 600/644 di unix**, cek overwrite. **RNG:** struct `OsRng` sendiri impl `rand_core` 0.10 `TryRng<Error=Infallible>` + marker `TryCryptoRng` berbasis `getrandom::fill` (blanket → `CryptoRng`) — dep baru `rand_core = "0.10"` (unifikasi ke 0.10.1 yg dipakai ssh-key, jadi tipe `CryptoRng` cocok). Return `{privatePath, publicPath, publicOpenssh}`. Registrasi di `lib.rs` (`mod keygen` + `generate_handler!`). **Frontend `GenerateKeyModal.tsx`** (baru): pilih type (Ed25519/RSA+bits) · comment (default `user@host`) · passphrase+confirm · "Save to" (default `~/.ssh/id_*` via `@tauri-apps/api/path` `homeDir`/`join`, tombol Browse via dialog `save`) · Generate → `invoke` → tampilan hasil **public key + Copy** (`navigator.clipboard`) + path + instruksi authorized_keys. `onGenerated` → `setSsh({keyPath, authMethod:"key", keyPassphrase})` (passphrase ikut alur keychain 26A saat profil disimpan). Tombol **"Generate key…"** di section key `ProfileEditor.tsx`. **Capability:** tak berubah — `dialog:default` sudah mencakup `save`, `core:default` mencakup `path`. **Verifikasi:** `tsc` + `pnpm build` (vite) lolos; `cargo check` **dan** `--features msstore` lolos (hanya warning lama). Sisa: uji native — generate Ed25519/RSA, cek file & permission, connect pakai key hasil generate, dan (Windows/MSIX) generate di paket Store.
- **26A ✅ (implementasi; uji native pending, 2026-08-26)** — Pakai key existing + strip sync. **`profiles.ts`**: field baru `SshOptions.keyPassphrase` (default `""`) + helper `keyPassphraseId(id)` = `"<id>#keyPassphrase"`; `sshOpenFromProfile` kini ambil passphrase dari keychain (`secretGet(keyPassphraseId(p.id))`) dan kirim di objek auth `{type:"key",path,passphrase}` — untuk koneksi **utama** dan **jump host** (buang `passphrase:null` hardcode). Backend `ssh.rs` **tak berubah** (sudah dukung passphrase via `load_secret_key`). **`App.tsx` `saveProfile`**: passphrase diperlakukan seperti password — dipindah ke keychain (`secretSet(keyPassphraseId(id), …)` hanya bila diisi), di-strip dari profil tersimpan (`keyPassphrase:""`), dan `deleteProfile` hapus `secretDelete(keyPassphraseId(id))`. **`ProfileEditor.tsx`** (GeneralTab, section key/auto): tombol **Browse** (reuse `tauri-plugin-dialog` `open`) di samping input path, input **Key passphrase** (`type=password`, `value ?? ""` utk profil lama), hint "key tinggal di device ini". Hint password lama Indonesia→English (selaras §16). **§26.10 di `sync.rs`**: `get_sync_payload` blank-kan `ssh.keyPath` tiap profil sebelum upload (passphrase sudah otomatis lokal — hanya secret `id`/`did` yang dikumpulkan, bukan `#keyPassphrase`); `apply_sync_payload` snapshot `local_key_path_map` lalu `with_local_key_paths` inject balik keyPath lokal per-id saat pull (profil baru dari device lain → keyPath kosong). **Verifikasi:** `pnpm install` (plugin dialog/autostart/notification ter-restore), `tsc` lolos, `cargo check` **dan** `--features msstore` lolos (hanya warning lama). Sisa: uji native — connect key ber-passphrase, dan push→pull antar-device pastikan key kosong di device baru tapi tetap ada di device asal.
