...
- **25C — Uji native menyeluruh** (checklist §25.8) di mesin user. Pakai identitas asli dari Partner Center (§25.3).
  ...
  - **25C-2 ⬜** — Uji native oleh user: (a) install lokal (self-sign identitas asli) + jalankan checklist §25.8 termasuk reboot utk StartupTask start-hidden; (b) opsional jalankan **WACK** (Windows App Certification Kit / `appcert.exe`) sbg pre-check sertifikasi.
...
**Status:** **25A ✅ + 25B ✅** (25B-1/25B-2/25B-3, 2026-08-23, branch `ms-store`) — pipeline MSIX + build variant `msstore`: updater mati, UI updater disembunyikan, autostart via StartupTask WinRT, start-hidden saat login (deteksi aktivasi StartupTask). Sisa: **25C** (uji native dgn identitas Partner Center) & **25D** (submit).
