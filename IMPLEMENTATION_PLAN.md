...
**Status:** **25A ✅ + 25B ✅** (25B-1 + 25B-2, 2026-08-23, branch `ms-store`) — pipeline MSIX + build variant `msstore` (updater mati, UI updater disembunyikan, autostart via StartupTask WinRT). Sisa: **25C** (uji native dgn identitas Partner Center; termasuk keterbatasan start-hidden StartupTask) & **25D** (submit).
